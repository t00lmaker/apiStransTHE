...
public class RotaService {
	...
	// obter paradas proximas de A e B, descobrir linhas que saem das proximas de a, até as proximas de B
	public List<Rota> getRotas(PontoDeInteresse a, PontoDeInteresse b) {
		List<Parada> origens = maisProximas(a, 500, 3);
		List<Parada> destinos = maisProximas(b, 500, 3);
		List<Rota> rotas = getRotas(origens, destinos);
		// TODO adicionar trechos à pé...
		for (Rota rota : rotas) {
	...
	private List<Rota> getRotas(List<Parada> origens, List<Parada> destinos) {
	...
	// retornar a primeira parada da linha que é destino e ocorre após ter passado pela origem
